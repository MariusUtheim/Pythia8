...
bool LowEnergyHadHad::collide( int i1, int i2, int typeIn, Event& event,
  ...
  int type = typeIn;
  if (typeIn == 0)
    type = lowEnergySigma.pickProcess(id1, id2, eCM);
  ...
  if ((type >= 1 && type <= 6) || (type == 8)) {
    ...
    switch (type) {
      case 1: 
        return false; // if (!nondiff()) return false;
        break;
      
      case 2: case 3: case 4: case 5:
        return false; // if (!eldiff(type)) return false;
        break;

      case 6:
        return false; // if (!annihilation()) return false;
        break;
...
//    }
    ...
    for (int i = 3; i < leEvent.size(); ++i) {
      ...
      event[iNew].mothers(mother2, mother1);
      //if (leEvent[i].mother1() != 0)
      //  event[iNew].mother1(leEvent[i].mother1() + motherOffset);
      //if (leEvent[i].mother2() != 0)
      //  event[iNew].mother2(leEvent[i].mother2() + motherOffset);
    }
  }
...
bool LowEnergyHadHad::resonance(int idRes) {
  ...
  // Create the resonance
  int iNew = leEvent.append(idRes, -999, 1,2, 0,0,
