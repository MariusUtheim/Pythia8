// hubifyfile.cc is a part of the PYTHIA event generator.
// Copyright (C) 2019 Torbjorn Sjostrand.
// PYTHIA is licenced under the GNU GPL v2 or later, see COPYING for details.
// Please respect the MCnet Guidelines, see GUIDELINES for details.

// Execute with "private/hubifyfile filename" from main directory.
...
// -- perform string replacements Info -> InfoHub and info -> infoHub.

// By default hubifyfile only works on .h, .cc, .cmnd and .xml files,
...
// Replace strings inside file; Info -> InfoHub and info -> infoHub.
int kindInfo = 15;
string infoOld[15] = { "Info    ", "Info ", "Info.", "Info(", 
  "Info*    ", "Info*", "Info_", "Info:", "info.", "info;", 
  "info,", "info)", "info =", "infoPtr    ", "infoPtr" }; 
string infoNew[15] = { "InfoHub ", "InfoHub ", "InfoHub.", "InfoHub(", 
  "InfoHub* ", "InfoHub*", "InfoHub_", "InfoHub:", "infoHub.", "infoHub;", 
  "infoHub,", "infoHub)", "infoHub = ", "infoHubPtr ", "infoHubPtr"  };
int    infoLen[15] = { 8, 5, 5, 5, 9, 5, 5, 5, 5, 5, 5, 5, 6, 11, 7};

//--------------------------------------------------------------------------

// Forward reference.
bool hubifyFile(string fileName, bool doPad);
...
int main(int argc, char* argv[]) {
  ...
  for (unsigned int i = 0; i < files.size(); ++i) {
    ...
    if (isCode || isXML || doForce) {
      cout << "\n Begin hubifying file " << fileName << endl;
      if (!hubifyFile( fileName, isXML)) problemFiles.push_back( fileName);
    } else {
...
bool hubifyFile(string fileName, bool doPad) {
  ...
  for (unsigned int i = 0; i < lines.size(); ++i) {
    ...
    // Replacement of Info (info) for InfoHub (infoHub).
    int cmtBeg  = (line.find("//") == string::npos) ? 200 : line.find("//"); 
    for (int j = 0; j < kindInfo; ++j) {
      bool found = true;
      int pos = 0;
      while (found) {
        if (line.find(infoOld[j], pos) != string::npos) {
          int infoBeg = line.find(infoOld[j], pos);
          if ( (infoBeg == 0 || !isalpha(line[infoBeg-1])) 
            && infoBeg < cmtBeg ) {
            line.replace( infoBeg, infoLen[j], infoNew[j]);
            ++nReplace;
          } else pos = infoBeg + 1;
        } else found = false;
      }
