...
public:
  ...
  int nFinal(bool chargedOnly = false) const { int nFin = 0;
    for (int i = 0; i < size(); ++i) if (entry[i].isFinal()
    && (!chargedOnly || entry[i].isCharged())) ++nFin; return nFin; }
  ...
  double dphiAbs(int i1, int i2) const {
    ...
    if (dPhiTmp > M_PI) dPhiTmp = 2. * M_PI - dPhiTmp; return dPhiTmp; }
