// test252.cc is a part of the PYTHIA event generator.
...
public:
  ...
  // Constructor.
  LowEnergyHadHad() : infoPtr(), particleDataPtr(), rndmPtr(), pythiaPtr() {}

  // Initialize the class.
  bool init(Info* infoPtrIn, Settings& settings,
    ParticleData* particleDataPtrIn, Rndm* rndmPtrIn, Pythia* pythiaPtrIn);
...
private:
  ...
  // Temporary pointer to Pythia itself; to be removed when integrated.
  Pythia*       pythiaPtr;
...
bool LowEnergyHadHad::init(Info* infoPtrIn, Settings& settings,
  ...
  subcollision.init( "(subcollision)", particleDataPtr);
...
bool LowEnergyHadHad::collide( int typeIn, int i1, int i2, Event& event) {
  ...
  // Reset subcollision event record. Add incoming hadrons as beams.
  subcollision.reset();
  subcollision.append( event[i1]);
  subcollision.append( event[i2]); 
  subcollision[1].status( -12);
  subcollision[2].status( -12);
  ...
  int subcolSize = subcollision.size();
  pythiaPtr->simpleHadronization( subcollision);
  for (int i = subcolSize; i < subcollision.size(); ++i) 
    event.append( subcollision[i]);
  
  // Boost from collision rest frame to event frame. Set status and mothers.
...
bool LowEnergyHadHad::nondiff( int i1, int i2, Event& event) {
  ...
  subcollision.append(  idc1, 63, i1, 0, 0, 0, newc,     0,  pc1,  mc1);
  subcollision.append( idac2, 63, i2, 0, 0, 0, 0,     newc, pac2, mac2);
  subcollision.append(  idc2, 63, i2, 0, 0, 0, newc + 1, 0,  pc2,  mc2);
  subcollision.append( idac1, 63, i1, 0, 0, 0, 0, newc + 1, pac1, mac1);
...
bool LowEnergyHadHad::eldiff( int type, int i1, int i2, Event& event) {
  ...
  if (excite1) { 
    ...
    int newc = event.nextColTag();  
    subcollision.append(  idc1, 63, i1, 0, 0, 0, newc,     0,  pc1,  mc1);
    subcollision.append( idac1, 63, i1, 0, 0, 0, 0,     newc, pac1, mac1);

  // Simple copy if not excited, and set momentum as in collision frame.
  } else {
    int iNew = subcollision.append( event[i1]);
    subcollision[iNew].p( pA);
    subcollision[iNew].vProd( 0., 0., 0., 0.);
  }
  ...
  if (excite2) { 
    ...
    pac2.bst(pB);
    int newc = event.nextColTag();  
    subcollision.append(  idc2, 63, i2, 0, 0, 0, newc,     0,  pc2,  mc2);
    subcollision.append( idac2, 63, i2, 0, 0, 0, 0,     newc, pac2, mac2);

  // Simple copy if not excited, ad set momentum as in collision frame.
  } else {
    int iNew = subcollision.append( event[i1]);
    subcollision[iNew].p( pB);
    subcollision[iNew].vProd( 0., 0., 0., 0.);
  }
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
   ...
   if (event[i1].isFinal() && event[i1].isHadron()) {
     ...
     && (event[i1].p() + event[i2].p()).mCalc() > 10.) {
       ...
       //int kind = 5;
       if (iEvent < nList) cout << " collision between " << i1 << " and " 
          << i2 << " is of kind " << kind << endl;
       LEHH.collide( kind, i1, i2, event);
       if (iEvent < nList) LEHH.subcollision.list();
       ++ncollide;
       break;
     }
   } 
   if (iEvent < nList) cout << " event contained " << ncollide 
     << " collisions " << endl;
