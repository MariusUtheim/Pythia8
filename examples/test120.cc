...
int main() {
  ...
  Hist eW(    "energy spectrum of W+-",            100, 0., 2500.);
  Hist eZ(    "energy spectrum of Z0",             100, 0., 2500.);
  Hist eGamma("energy spectrum of photons",        100, 0., 2500.);
  Hist eE(    "energy spectrum of e+ and e-",      100, 0., 2500.);
  Hist eP(    "energy spectrum of p and pbar",     100, 0., 2500.);
  Hist eNu(   "energy spectrum of neutrinos",      100, 0., 2500.1);
  Hist eRest( "energy spectrum of rest particles", 100, 0., 2500.);
...
/*
    // Search for events with W/Z being produced.
    for (int i = 0; i < pythia.event.size(); ++i)
    if (pythia.event[i].idAbs() == 24 || pythia.event[i].id() == 23) {
      if (iListWZ++ < nListWZ) pythia.event.list();
      if (pythia.event[i].idAbs() == 24) eW.fill(pythia.event[i].e());
      else                               eZ.fill(pythia.event[i].e());
    }
*/
  ...
  pythia.stat();
  cout << eW << eZ << eGamma << eE << eP << eNu << eRest;
