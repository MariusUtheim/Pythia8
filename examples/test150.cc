...
complex amplitudeABMST( double s, double t, double ispp = true,
  ...
  if (useCoulomb && t < 0.) {
    ...
    //?? Phase convention still to be checked, pp vs ppbar!
...
int main() {
  ...
  // Variables used in various contexts.
  int nEv;
  double eCM, s, t;
  ...
  // Simple t slopes; approximation used in RPP for Coulomb phase.
  cout << "\n   eCM       Bsimple_pp         Bsimple_ppbar "
       << "\n           ABMST      RPP      ABMST      RPP" << endl;
  for (int iE = 0; iE < nE; ++iE) {
    eCM               = eCMval[iE];
    s                 = eCM * eCM;
  ...
  */
  ...
  // True and Monte Carlo dsigma_el/dt spectra, with/without Coulomb.
  nEv             = 100000;
  eCM             = 20.;
  s               = eCM * eCM;
  double tAbsMin  = 0.0001;
  double tAbsMax  = 0.1;
  bool useCoulomb = true;
  //double tAbsMin  = 0.0;
  //double tAbsMax  = 5.0;
  //bool useCoulomb = false;
  Hist dsigHPPABMST( "log10 dsigma/dt pp ABMST", 100, 0.0, tAbsMax);
  Hist dsigHPbABMST( "log10 dsigma/dt ppbar ABMST", 100, 0.0, tAbsMax);
  Hist dsigHPPRPP( "log10 dsigma/dt pp RPP", 100, 0.0, tAbsMax);
  Hist dsigHPbRPP( "log10 dsigma/dt ppbar RPP", 100, 0.0, tAbsMax);
  Hist dsigMCPPABMST( "log10 dsigma/dt MC pp ABMST", 100, 0.0, tAbsMax);
  Hist dsigMCPbABMST( "log10 dsigma/dt MC ppbar ABMST", 100, 0.0, tAbsMax);
  Hist dsigMCPPRPP( "log10 dsigma/dt MC pp RPP", 100, 0.0, tAbsMax);
  Hist dsigMCPbRPP( "log10 dsigma/dt MC ppbar RPP", 100, 0.0, tAbsMax);
  Hist dsigRPPABMST( "dsigma/dt ratio pp ABMST", 100, 0.0, tAbsMax);
  Hist dsigRPbABMST( "dsigma/dt ratio ppbar ABMST", 100, 0.0, tAbsMax);
  Hist dsigRPPRPP( "dsigma/dt pp ratio RPP", 100, 0.0, tAbsMax);
  Hist dsigRPbRPP( "dsigma/dt ppbar RPP", 100, 0.0, tAbsMax);
  for (int iT = 0; iT < 1000; ++iT) {
    t = -0.001 * (iT + 0.5) * tAbsMax;
    dsigHPPABMST.fill( -t, dsigmadtABMST( s, t, true, useCoulomb) );
    dsigHPbABMST.fill( -t, dsigmadtABMST( s, t, false, useCoulomb) );
    dsigHPPRPP.fill(   -t, dsigmadtRPP( s, t, true, useCoulomb) );
    dsigHPbRPP.fill(   -t, dsigmadtRPP( s, t, false, useCoulomb) );
  }
  cout << endl;
  double tNow;
  for (int iEv = 0; iEv < nEv; ++iEv) {
    tNow = picktABMST( s, rndm, true, useCoulomb, tAbsMin);
    dsigMCPPABMST.fill( -tNow );
    tNow = picktABMST( s, rndm, false, useCoulomb, tAbsMin);
    dsigMCPbABMST.fill( -tNow );
    tNow = picktRPP( s, rndm, true, useCoulomb, tAbsMin);
    dsigMCPPRPP.fill( -tNow );
    tNow = picktRPP( s, rndm, false, useCoulomb, tAbsMin);
  ...
  */
  ...
  s   = eCM * eCM;
  double tAbsMaxC = 1.0;
  Hist dsigCPPABMST( "Coulomb corr dsigma/dt pp ABMST", 100, 0.0, tAbsMaxC);
  Hist dsigCPbABMST( "Coulomb corr dsigma/dt ppbar ABMST", 100, 0.0, tAbsMaxC);
  Hist dsigCPPRPP( "Coulomb corr dsigma/dt pp RPP", 100, 0.0, tAbsMaxC);
  Hist dsigCPbRPP( "Coulomb corr dsigma/dt ppbar RPP", 100, 0.0, tAbsMaxC);
  for (int iT = 0; iT < 100; ++iT) {
    t = -0.01 * (iT + 0.5) * tAbsMaxC;
